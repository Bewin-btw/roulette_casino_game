...
CHIPS = [1, 2, 10, 50, 100, 500, 1000, 5000]
...
class Score:
    ...
    def update(self, total_score):
        ...
        for i in range(6):
            scores[5 - i] = int(n % 10)
            n = n // 10
        write_txt(self.t, self.X, self.Y + 10, "Balance:", font=FONT_SCORE_TITLE)
        for i in range(6):
            self.t.up()
...
class Chip:
    ...
    def update(self, state):
        self.t.clear()
        pencols = [COLOR_BLACK] * len(CHIPS)
        bgcols = [COLOR_GREEN] * len(CHIPS)
        for i in range(len(CHIPS)):
        ...
        if self.tot_score >= chip_value:
            for i in range(44):
                if in_range(x, y,
                            ...
                            self.bet_pos[i][0] + self.bet_pos[i][2],
                            self.bet_pos[i][1] + self.bet_pos[i][3]):
                    self.bets[i] += chip_value
                    self.tot_score -= chip_value
...
class Game:
    def __init__(self):
        ...
        self.state = STARTUP
        self.history_turtle = Turtle()
        self.history_turtle.hideturtle()
        self.history_turtle.penup()
        self.history_turtle.goto(-200, 200)
    ...
    def update_history(self, ro_idx):
        self.history.append(ro_idx)
        self.history = self.history[-5:]

    def draw_history(self):
        self.history_turtle.clear()
        self.history_turtle.goto(-350, 200)
        self.history_turtle.write("History:", font=FONT_SCORE_TITLE)

        for i, result in enumerate(reversed(self.history[-5:])):
            self.history_turtle.goto(-300, 180 - (i * 20))
            history_entry = f"Round {len(self.history) - i}: {RONUMS[result]}"
            self.history_turtle.write(history_entry, font=FONT_SCORE)
